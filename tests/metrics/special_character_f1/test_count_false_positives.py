...
def test_simple_example():
    ...
    assert metric.count_false_positives(true_character_count, transcription_character_count) == 1
...
def test_empty_transcription():
    ...
    transcription_character_count = Counter(transcription)
    metric = SpecialCharacterF1(special_characters)
    assert metric.count_false_positives(true_character_count, transcription_character_count) == 0
...
def test_empty_gt():
    ...
    transcription_character_count = Counter(transcription)
    metric = SpecialCharacterF1(special_characters)
    assert metric.count_false_positives(true_character_count, transcription_character_count) == 3
